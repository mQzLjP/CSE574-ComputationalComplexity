...
\begin{definition}
	A language $L$ is a set of strings that $L\subseteq \Sigma^*$, where $\Sigma$ is a finite alphabet (often $\{0,1\}$ bits.)
\end{definition}
...
\begin{definition}
	$L \in : \DTIME(T(n))$ if there's a deterministic Turing machine $M$ that decides $L$ in time $O(T(n))$.
	$ \P := \bigcup_{c\ge 1} \DTIME(n^c)$ is the class of problems that can be solved in polynomial time.
\end{definition}

It's easy to see that $\text{PRIMES}\subseteq \P$.
